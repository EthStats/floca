...
var Server = function( config, callback ) {
	...
	var iopts;
	if( self.config.amqp.connectURL ){
	...
	else if( self.config.nsq.nsqdHost ){
		console.log('Using NSQ...');
		var nsqConfig = { Barrel: Nsq.Barrel, barrel: self.config.nsq, logger: self.logger, idLength: 32, name: self.config.NODE_APP_NAME || self.config.entities.appName };
...
ServerProto.buildUpRest = function( app ){
	...
	if( self.config.server.jwt.secret && self.config.server.jwt.acquireURI )
		RestMaker.buildUp( radiation, rester, harcon, { jwt: jwt, acquireURI: self.config.server.jwt.acquireURI, pathToIgnore: pathToIgnore } );

	if( self.config.extendREST && _.isFunction(self.config.extendREST) ){
		self.config.extendREST( self.config, rester, pathToIgnore, harcon );
	}
