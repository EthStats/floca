...
ServerProto.buildUpRest = function( app ){
	...
	if( self.config.server.jwt.secret && self.config.server.jwt.acquire )
		RestMaker.buildUp( radiation, rester, harcon, { jwt: jwt, pathToIgnore: pathToIgnore } );
...
ServerProto.serve = function( callback ){
	...
	var jwtKey = self.config.server.jwt.key || 'x-warper-jwt';
	self.originHeaders ={
