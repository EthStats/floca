...
module.exports = {
	writeComponentTests: function( component, restIT, websocketIT, callback ){
		...
		component.services.forEach( function(service){
			if( component.rest ){
				rest = rest.concat( '\n' + restIT.replace( '$$$SERVICE$$$', service ).replace( '$$$DIVISION$$$', component.division?'/'+component.division:'' ).replace( '$$$COMP_NAME$$$', component.name ).replace( '$$$SERVICE_NAME$$$', service ) + '\n' );
			}
	...
	generateTests: function( port, testFolder ){
		var self = this;
		start( port, function(err, res){
