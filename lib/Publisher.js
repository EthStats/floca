		...
		newFiles.forEach( function(newFile){
			...
			if( fs.existsSync( newFile ) ){
				...
				if( !component.adequate || component.adequate() ){
					...
					if( self.globalConfig.entities.configurator )
						self.ignite( self.globalConfig.entities.configurator + '.config', self.globalConfig.entities.appName, component.name, function(err, config){
							self.harcon.addicts( component, config || self.configs[component.name] || self.globalConfig[component.name], fn );
						});
					else
						self.harcon.addicts( component, self.configs[component.name] || self.globalConfig[component.name], fn );
				}
			} else
				self.harcon.detracts( path.basename( newFile, '.js'), fn );
		} );
	},
